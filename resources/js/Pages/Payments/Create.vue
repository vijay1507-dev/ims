<template>
    <AppLayout>
        <div class="max-w-4xl mx-auto p-6">
            ...
            <div class="mb-6">
                <Link :href="form.redirect_customer_id ? `/customers/${form.redirect_customer_id}/edit` : '/payments'" class="inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-800 transition-colors mb-2">
                    <i class="fas fa-arrow-left mr-2 text-xs"></i> Back to {{ form.redirect_customer_id ? 'Customer Profile' : 'Payments Dashboard' }}
                </Link>
                <h2 class="text-2xl font-bold text-gray-900">Record Payment Transaction</h2>
                <p class="text-gray-600 mt-1">Manually insert invoice settlements and custom account billing charges.</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-xs border border-gray-200 overflow-hidden">
                <form @submit.prevent="submitForm">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        type="text"
                                        required
                                        placeholder="e.g. ****4242 or Stripe Gateway"
                    ...
                    <div class="px-6 sm:px-8 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
                        <Link
                            :href="form.redirect_customer_id ? `/customers/${form.redirect_customer_id}/edit` : '/payments'"
...
const form = useForm({
    customer_name: props.initial_customer,
    amount: 2450.00,
    payment_method: '****4242',
    method_type: props.paymentChannels && props.paymentChannels.length > 0 ? props.paymentChannels[0].code : 'visa',
    status: 'successful',
    invoice_ref: '',
...
const initialCustomerMatch = props.customers.find(c => c.name === props.initial_customer);
const selectedCustomerId = ref(initialCustomerMatch ? initialCustomerMatch.id : '');

const searchQuery = ref(props.initial_customer || '');
...
watch(selectedCustomerId, (newId) => {
    ...
    // Changing the customer must reset the selected sub-customer.
